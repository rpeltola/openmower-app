...
import {Box, Card, CardContent, Stack, useMediaQuery, useTheme} from '@mui/material';
...
export default function HistoryPage() {
  ...
  // Default to the most recent job once the list arrives, so the map/event view isn't empty
  // on first visit. Doesn't fight a user's own selection (only fires while nothing is picked).
  useEffect(() => {
  ...
  const jobListCard = (
    <Card sx={{...outerCardStyles(theme), flex: isMobile ? undefined : '0 1 45%', minHeight: 0, display: 'flex'}}>
      <CardContent sx={{flex: 1, minHeight: 0, display: 'flex', height: isMobile ? 320 : undefined}}>
        <JobList jobs={jobs} loading={jobsLoading} selectedJobId={selectedJobId} onSelectJob={handleSelectJob} />
      </CardContent>
    </Card>
  );
  ...
  return (
    <Page sx={{height: isMobile ? undefined : 'calc(100% - 16px)'}}>
      <PageHeader title="History" subtitle="Review past mow jobs, the map they used, and their events">
        <HeaderStat icon={<HistoryIcon />} value={jobs.length} label="Jobs in range" />
